define([
    ...
    'dust!pages/customer-service-gift-certificate/partials/product-tiles',
    ...
    var _updatePricesAndErrors = function() {
        $content.find('.js-product-price').each(function() {
            ...
            priceTemplate(data, function(err, html) {
                ...
                // Error message
                if (desktopError) {
                    $self.parent().find('.js-error-text')
                        .text(desktopError)
                        .prop('hidden', false);
                }
            });
        ...
        if (Adaptive.evaluatedContext.isWebsphere) {
            ...
            Utils.afterAppendChild('#gwt_products_display', function($container) {
                ...
                productTilesTemplate({items: data}, function(err, html) {
                    ...
                    $content.find('.c-heading > a').map(function(_, heading) {
                        var $heading = $(heading);
                        $heading.text($heading.text().replace(/\$[0-9]+/, ''));
                    });
